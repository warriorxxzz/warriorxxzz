    ...
    <article className="panel panel-hover group flex h-full flex-col overflow-hidden">
      ...
      <div className="flex flex-1 flex-col p-5">
        <div className="mb-2 flex items-start justify-between gap-3">
          ...
          {project.status && (
            <span
              className={`shrink-0 rounded-sm border px-2 py-0.5 font-mono text-[11px] ${statusStyles[project.status]}`}
            >
              {project.status}
            </span>
          )}
        ...
        <div className="flex items-center gap-4 border-t border-border pt-4">
          {project.githubUrl && (
            <a
              ...
              target="_blank"
              rel="noreferrer"
              className="flex items-center gap-1.5 font-mono text-xs text-muted hover:text-signal"
            >
